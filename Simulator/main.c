...
int main(int argc, char *argv[]) {
    ...
    float TsMLFQ1 = atoi(argv[4]);
    float TsMLFQ2 = atoi(argv[5]);
    float TsMLFQ3 = atoi(argv[6]);
    float BMLFQ = atoi(argv[7]);
    ...
    while (!feof(inputFile)) {
        ...
        if (fscanf(inputFile, "%s %f %f", pid, &startTime, &runTime) == 3) {
            numProcesses++;
        }
    }
    ...
    for (int i = 0; i < numProcesses; i++) {
        fscanf(inputFile, "%s %f %f", processes[i].pid, &processes[i].arrivalTime, &processes[i].jobTime);
    }
